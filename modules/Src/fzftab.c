...
static char *colnames[] = {"no", "fi", "di", "ln", "pi", "so", "bd", "cd", "or",
                           "mi", "su", "sg", "tw", "ow", "st", "ex", "lc", "rc",
                           "ec", "tc", "sp", "ma", "hi", "du", "sa", NULL};

/* Default values. */
static char *defcols[] = {
    "0",  "0",     "1;31",  "1;36",  "33",    "1;35",  "1;33", "1;33",  NULL,
    NULL, "37;41", "30;43", "30;42", "34;42", "37;44", "1;32", "\033[", "m",
    NULL, "0",     "0",     "7",     NULL,    NULL,    "0"};
...
static int bin_fzf_tab_compcap_generate(char *nam, char **args, Options ops,
                                        UNUSED(int func)) {
  int i;
  ...
  // get paramaters
  char **words = getaparam(args[0]), **dscrs = getaparam(args[1]),
       *opts = getsparam(args[2]);
  if (!words || !dscrs || !opts) {
  ...
  for (i = 0; words[i]; i++) {
    ...
    if (ftb_compcap.len == ftb_compcap.size) {
      ftb_compcap.array = zrealloc(ftb_compcap.array,
                                   (1024 + ftb_compcap.size) * sizeof(char *));
      ftb_compcap.size += 1024;
      memset(ftb_compcap.array + ftb_compcap.size - 1024, 0,
             1024 * sizeof(char *));
    }
  }
...
static struct file_color *fzf_tab_colorize(char *file) {
  ...
  if (color != NULL) {
    fc->fc_begin =
        ftb_strcat(NULL, 3, mode_color[COL_LC], color, mode_color[COL_RC]);
    fc->fc_end =
        ftb_strcat(NULL, 3, mode_color[COL_LC], "0", mode_color[COL_RC]);
  } else {
    fc->fc_begin = ztrdup("");
    fc->fc_end = ztrdup("");
  }
  ...
  if (symlink != NULL) {
    fc->sc =
        ftb_strcat(NULL, 7, mode_color[COL_LC], symcolor, mode_color[COL_RC],
...
static int bin_fzf_tab_candidates_generate(char *nam, char **args, Options ops,
                                           UNUSED(int func)) {
  int i, j;
  ...
  char **ftb_compcap = getaparam("_ftb_compcap"),
       **group_colors = getaparam("group_colors"),
       *default_color = getsparam("default_color"),
       *prefix = getsparam("prefix");
  ...
  for (i = 0; i < ftb_compcap_len; i++) {
    ...
    if (group) {
      ...
      char *color =
          group_index >= group_colors_len ? "" : group_colors[group_index - 1];
      // add prefix
      result = ftb_strcat(result, 11, group, bs, color, prefix, dpre, nul,
                          group, bs, desc, nul, dsuf);
    } else {
...
static struct builtin bintab[] = {
    BUILTIN("fzf-tab-compcap-generate", 0, bin_fzf_tab_compcap_generate, 0, -1,
            0, "io", NULL),
    BUILTIN("fzf-tab-candidates-generate", 0, bin_fzf_tab_candidates_generate,
            0, -1, 0, "i", NULL),
};
